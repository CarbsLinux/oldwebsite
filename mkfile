...
wiki:V:
	...
	git -C /tmp/wiki log -- $md | grep Date | sed 's/Date:/* **Last Edit:**/;1q' >> $md
	printf '* **Commit Message**: ' >> $md
	git -C /tmp/wiki log -1 '--pretty=%B' -- $md | awk 'NF' >> $md
	git -C /tmp/wiki log -- $md | grep Author | sed 's/Author:/* **Author:**/;1q' >> $md
	}
